...
public class FetchCmd implements RuntimeCommand {
    private final Logger m_logger = LoggerFactory.getLogger(FetchCmd.class);
    
    /**
     * JDBC resultset wrapper.
     * 
     * @author vsi
     */
    ...
    private class CsvRowHandler implements RowHandler {
        private final Logger m_logger = LoggerFactory.getLogger(CsvRowHandler.class);
        private CSVWriter m_csvWriter;

        public CsvRowHandler() throws Exception {
    ...
    private static class RowHandlerList {
        private final Logger m_logger = LoggerFactory.getLogger(RowHandlerList.class);
        private ArrayList<RowHandler> m_list = new ArrayList<RowHandler>(2);
        ...
        public void close() {
            for (RowHandler handler: m_list) {
                try {
                    ...
                    m_logger.warn("Failed to close handler", e);
                }
            }
        }
    }
    ...
    public void run(Context context) throws Throwable {
        m_logger.info(String.format(
                ...
                "type='%s', rowset='%s', overwrite='%s', delimiter='%s', quote='%s'",
        ...
        try {
            ...
            try {
                ...
                rowHandlers.close();
                m_logger.info(String.format("%d row(s) fetched", rowCount));
            } finally {
                close(rs);
            }
        } finally {
    ...
    private void close(MyResultSet rs) {
        try {
            ...
            m_logger.warn("Failed to close resultset", t);
        }
    }
