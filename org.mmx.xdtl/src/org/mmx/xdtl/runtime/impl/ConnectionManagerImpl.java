...
public class ConnectionManagerImpl implements ConnectionManager {
    private static final Logger logger = XdtlLogger.getLogger("xdtl.rt.connectionManager");
    ...
    private JdbcConnection createJdbcConnection(Connection modelCnn)
        ...
        if (modelCnn.getUser().length() > 0 && modelCnn.getPassword().length() > 0) {
            cnn = DriverManager.getConnection(modelCnn.getValue(), modelCnn.getUser(), modelCnn.getPassword());
        } else {
            cnn = DriverManager.getConnection(modelCnn.getValue());
        }
