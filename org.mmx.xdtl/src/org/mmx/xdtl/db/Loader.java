...
public class Loader {
	...
	public Loader(JdbcConnection cnn, String table, int batchSize, int commitRowCount) throws SQLException {
		m_cnn = cnn;
		...
		m_table = metaData.storesUpperCaseIdentifiers() ? table.toUpperCase() : table;
		init(metaData);

		m_initialAutoCommit = m_cnn.getAutoCommit();
		if (logger.isDebugEnabled()) {
		    String msg = "batch size=%d, initial autocommit=%s, commitRowCount=%d";
		    msg = String.format(msg, m_batchSize, m_initialAutoCommit ? "on" : "off", m_commitRowCount);
	...
	private void init(DatabaseMetaData metaData) throws SQLException {
		...
		try {
			while (rs.next()) {
				m_columns.add(new Column(rs.getInt("DATA_TYPE"), rs.getString("TYPE_NAME"), rs.getString("COLUMN_NAME")));
				//m_logger.debug(rs.getString("COLUMN_NAME") + " " + rs.getInt("DATA_TYPE") + " " + rs.getString("TYPE_NAME"));
				sql.append("?,");
			}
		} finally {
			close(rs);
	...
	public void loadRow(Object[] values) throws Exception {
		...
		if (m_rowNum % m_batchSize == 0) {
		    ...
		    if (m_commitRowCount != 0 && (m_rowNum - m_lastCommit >= m_commitRowCount)) {
	...
	private Object convert(Object object, Column col) throws Exception {
		...
		// m_logger.debug("About to convert column " + col.getName() + " with value " + (object == null ? "null" : object.toString()));
